...
class Requisicao(Base):
    ...
    unidade = Column(String, nullable=False, default="AREAL RECRIA")
...
# Migração leve: se a coluna "unidade" não existir ainda em uma tabela
# já criada antes dessa mudança, cria ela sem apagar nada.
with engine.connect() as conn:
    from sqlalchemy import text
    try:
        conn.execute(text("SELECT unidade FROM requisicoes LIMIT 1"))
    except Exception:
        conn.rollback()
        try:
            conn.execute(text(
                "ALTER TABLE requisicoes ADD COLUMN unidade VARCHAR DEFAULT 'AREAL RECRIA'"
            ))
            conn.commit()
        except Exception:
            conn.rollback()
...
def unidade_do_usuario(user):
    return usuarios.get(user, {}).get("unidade", "AREAL RECRIA")
...
def carregar_excel(unidade="AREAL RECRIA"):
    caminho = arquivo_materiais_da_unidade(unidade)
...
def materiais(request: Request):
    ...
    unidade = unidade_do_usuario(request.session["user"])

    try:
        df = carregar_excel(unidade)
    except FileNotFoundError:
        return pagina_erro("Arquivo de materiais não encontrado para esta unidade.")
...
<div class="page">
  <span class="eyebrow">Catálogo &middot; {UNIDADES.get(unidade, {}).get('nome', unidade)}</span>
  <h2>📦 Materiais</h2>
...
def req(request: Request):
    ...
    unidades_col = [c for c in df.columns if "UNID" in c]
    col_un = unidades_col[0] if unidades_col else None
...
<div class="page page-narrow">
  <span class="eyebrow">Setor: {request.session['user']} &middot; {UNIDADES.get(unidade, {}).get('nome', unidade)}</span>
...
def painel(request: Request, filtro: str = "TODOS"):
    ...
    unidade_admin = unidade_do_usuario(request.session["user"])

    db = get_db()
    try:
        todas = (
            db.query(Requisicao)
            .filter(Requisicao.unidade == unidade_admin)
            .order_by(Requisicao.id.desc())
            .all()
        )
    finally:
...
<div class="admin-shell">
  ...
  <div class="admin-content">
    <span class="eyebrow">Visão geral &middot; {UNIDADES.get(unidade_admin, {}).get('nome', unidade_admin)}</span>
...
def relatorio(request: Request, inicio: str = "", fim: str = ""):
    ...
    db = get_db()
    try:
        todas = db.query(Requisicao).filter(Requisicao.unidade == unidade_admin).all()
    finally:
        db.close()
...
<div class="admin-shell">
  {sidebar("relatorio")}
  <div class="admin-content">
    <span class="eyebrow">Comparativo por período &middot; {UNIDADES.get(unidade_admin, {}).get('nome', unidade_admin)}</span>
